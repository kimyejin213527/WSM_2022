...
const showToday = () => {
    ...
    let day = now.getDay();
    const namesOfTheDaysOfTheWeek_arry = ['일', '월', '화', '수', '목', '금', '토',];
    //console.log(year, month, date, day, namesOfTheDaysOfTheWeek_arry[day]);

    //문자열 형식 맞추고
    let title = `${year}.${month}.${date}.${namesOfTheDaysOfTheWeek_arry[day]}`;
    //console.log(title);

    //HTML에 표시하자
    let caedDateDivs = document.querySelectorAll(".card-date");
